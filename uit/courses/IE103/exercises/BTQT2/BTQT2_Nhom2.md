...
Các thiết kế bám sát theo phương pháp luận và ký hiệu "cổ điển" (ký hiệu Chen cho ERD) được trình bày trong tài liệu `IE103-lectures-ch02a.pdf`.
...
1. **Bài toán 1: Quản lý Đề tài Tốt nghiệp**
    
    - 1.1. Phân tích Thực thể và Mối kết hợp
        
    - 1.2. Task 1: Mô hình ERD (Mức Quan niệm)
        
    - 1.3. Task 2: Mô hình Class Diagram (Mức Quan niệm)
        
    - 1.4. Task 3: Mô hình Logic (Mức Logic)
        
2. **Bài toán 2: Quản lý Cửa hàng Xe máy**
    
    - 2.1. Phân tích Thực thể và Mối kết hợp
        
    - 2.2. Task 1: Mô hình ERD (Mức Quan niệm)
        
    - 2.3. Task 2: Mô hình Class Diagram (Mức Quan niệm)
        
    - 2.4. Task 3: Mô hình Logic (Mức Logic)
...
Dưới đây là mô tả ERD theo ký hiệu Chen (hình thoi cho mối kết hợp).
...
- **ERD Đã sửa lỗi:**
    ...
    - **Giải thích:** Bản số `(1,3)` (của Đề tài) được đặt ở phía `SINHVIEN`. Bản số `(1,2)` (của Sinh viên) được đặt ở phía `DETAI`.
...
6. **THUCHIEN** (`MaSV` (PK, FK), `MaDT` (PK, FK), `LanBaoVe`, `DiemHD`, `DiemPB`, `DiemCT`)
    ...
    - **Ghi chú (Sửa đổi logic):** Theo nghiệp vụ "đề tài lần 1 phải khác lần 2", một `SINHVIEN` không thể làm cùng 1 `DETAI` 2 lần. Do đó, khóa chính là cặp `(MaSV, MaDT)`. `LanBaoVe` (1 hay 2) chỉ là thuộc tính mô tả cho lần thực hiện đó của sinh viên.
...
- `[BAOHANH]` ---(0,n)---<CHITIET_BH>---(0,n)--- `[LINHKIEN]`
    ...
    - Mối kết hợp `CHITIET_BH` có thuộc tính: `GiaTien`.
